# Context Contract — CC-001 v1.1

**Document ID:** CC-001
**Status:** DRAFT v1.1
**Owner:** IT Architecture / Legal / Procurement
**Last Updated:** April 7, 2026
**Applies To:** OptiChain vendor onboarding pipeline — all agents and retrieval operations within a single pipeline run.
...
| §4 | Source Initialization Manifest | Exactly which sources and versions are in scope. The PROVISIONAL flag originates here. |
| §5 | Authority Hierarchy & Conflict Resolution | Which sources override which. How conflicts are resolved. |
| §6 | Retrieval Method & Endpoint Permissions | How each source is retrieved and which agents are permitted at which index. |
| §7 | Freshness & Versioning Policy | Source-class-specific staleness and version-pinning rules. |
| §9 | Bundle Admissibility Requirements | Minimum evidence each agent needs before its bundle is admissible. |
| §12 | Determination Evidence Sufficiency Thresholds | When retrieved evidence is sufficient to support a determination status. |
| §13 | Escalation Ownership | Which escalation types are triggered by retrieval conditions and who owns resolution. |
| §14 | Citation Requirements | What every determination and audit log entry must cite. |

---

## 1. Purpose & Scope

This Context Contract governs all retrieval operations, source access, evidence handling, provenance requirements, and conflict resolution for the AI-assisted vendor onboarding pipeline at Lichen Manufacturing. It is the authoritative reference for which sources agents are permitted to access, how evidence must be structured before downstream use, and what triggers escalation or provisional status at the evidence layer.

This document must be finalized and locked before orchestration is built.

- Pipeline step-control behavior (execution graph, gate logic, routing decisions) → Design Doc
- Agent behavioral rules (DOs, DON'Ts, output format enforcement) → Agent Spec

**GOVERNANCE BOUNDARY:** This document governs source admissibility, evidence sufficiency, provenance, retrieval permissions, freshness, conflict handling, and audit triggers. It does not govern pipeline step sequencing, agent behavioral rules, or output schema definitions.
...
## 3. Status Model — Context Contract Alignment

The pipeline status model is defined in Design Doc §10. The Context Contract adds one clarification: how PROVISIONAL propagates through the evidence chain.

### 3.1 PROVISIONAL as a First-Class Pipeline Status

PROVISIONAL is a first-class pipeline status, co-equal with COMPLETE, ESCALATED, and BLOCKED. It is not a determination-level modifier that rolls up to ESCALATED. When any determination carries a PROVISIONAL evidence flag, the overall pipeline status is PROVISIONAL until resolved.

- This contract governs when evidence conditions create a PROVISIONAL flag.
- The Design Doc governs how the pipeline emits and routes that status.

### 3.2 BLOCKED vs. ESCALATED — Unified Convention

These two statuses are distinct and must not be conflated.

| STATUS | DEFINITION AND USAGE |
|--------|----------------------|
| **BLOCKED** (workflow state) | The pipeline cannot proceed on a step because required evidence is entirely absent — questionnaire not submitted, required upstream agent unresolved, mandatory source unavailable. A BLOCKED step has no determination to escalate because there is no evidence to reason over. |
| **ESCALATED** (resolution mode) | The pipeline has evidence but cannot resolve the determination from that evidence alone — a conflict exists, the case falls outside defined rules, or human judgment is explicitly required. ESCALATED always has a resolution owner and a minimum evidence payload. It is a resolution mode that may co-occur with BLOCKED. |
| **Combined expression** (when applicable) | When a step is operationally halted AND requires a human decision, express both: `workflow_state = BLOCKED`, `resolution_mode = ESCALATED_TO_[OWNER]`. Example: multiple questionnaire versions detected → pipeline is BLOCKED and conflict is ESCALATED_TO_PROCUREMENT. |

**CONSISTENCY REQUIREMENT:** All sections of this document, the Design Doc, and the Agent Spec must use BLOCKED and ESCALATED according to this convention.

---

## 4. Source Initialization Manifest

The Supervisor Agent locks a source manifest at pipeline initialization. This manifest records exactly which sources, at which versions, are in scope for this run. Any source not on the manifest is out-of-scope for the duration of the run. Mid-run manifest changes are not permitted.

| SOURCE ID | SOURCE NAME | VERSION | CONFIRMED BY (ROLE) | CONFIRMATION TIMESTAMP | STATUS |
|-----------|-------------|---------|---------------------|------------------------|--------|
| ISP-001 | IT Security Policy v4.2 | 4.2 | IT Security (K. Whitfield) | [to be confirmed] | **PROVISIONAL** |
| DPA-TM-001 | DPA Legal Trigger Matrix v2.1 | 2.1 | Legal (General Counsel) | [to be confirmed] | **PROVISIONAL** |
| PAM-001 | Procurement Approval Matrix v3.0 | 3.0 | Procurement (Director) | [to be confirmed] | **PROVISIONAL** |
| VQ-OC-001 | OptiChain Vendor Questionnaire | Submission rev. 1 | Procurement (Intake) | [on submission] | **PENDING** |
| PVD-001 | Prior Vendor Decisions / Precedent Log | Current at init. | IT Arch. / Platform | [at initialization] | **CONFIRMED** |
| SLK-001 | Slack / Meeting Thread Notes | Export at init. | Procurement | [at initialization] | **CONFIRMED** |

**PROVISIONAL INITIALIZATION:** Any source with `manifest_status = PROVISIONAL` causes all determinations that depend on that source to carry a PROVISIONAL evidence flag. The PROVISIONAL flag propagates through the evidence chain, not the workflow chain.

---

## 5. Source Authority Hierarchy

Sources are ranked into four authority tiers. When a retrieval bundle includes evidence from multiple tiers, higher-tier evidence takes precedence. Agents may not cite a lower-tier source as a primary citation when a higher-tier source addresses the same question.
...
| **Tier 3 — Governed Precedent** (Supplementary; informs but does not override Tier 1) | Prior Vendor Decisions / Precedent Log (PVD-001) |
| **Tier 4 — Low-Authority Supplemental** (Never primary citation; suppressed on any conflict with Tier 1–3) | Slack / Meeting Thread Notes (SLK-001) |

### 5.1 Conflict Resolution Protocol

| CONFLICT TYPE | RESOLUTION RULE | ESCALATION / BLOCKED CONVENTION |
|---------------|-----------------|----------------------------------|
| Tier 4 conflicts with Tier 1, 2, or 3 | Suppress Tier 4 source from bundle; log suppression in retrieval manifest with reason: `AUTHORITY_SUPPRESSED`; proceed with higher-tier evidence | No escalation — automatic suppression. No workflow impact. |
| Tier 4 thread A conflicts with Tier 4 thread B | Suppress both conflicting threads from bundle; log both suppressions | No escalation unless Tier 4 is the only available evidence on the point. If so: determination is PROVISIONAL. |
| Tier 3 conflicts with Tier 1 | Override: Tier 1 is operative; Tier 3 is contextual only. Log discrepancy in retrieval manifest. | No escalation — automatic override. Note the conflict in determination output. |
| Tier 3 conflicts with Tier 2 (precedent vs. questionnaire field) | Questionnaire field (Tier 2) is the operative fact. Precedent is contextual. | No escalation — automatic. Log in retrieval manifest. |
| Tier 1 source A conflicts with Tier 1 source B (e.g., policy clause vs. matrix row) | Neither source is suppressed. Both must be cited in the evidence payload. | `workflow_state = BLOCKED` (affected determination cannot resolve); `resolution_mode = ESCALATED_TO_[both domain owners]` |
| Multiple questionnaire versions detected | No version may be used until authoritative version is selected. | `workflow_state = BLOCKED` (all downstream steps); `resolution_mode = ESCALATED_TO_PROCUREMENT` |

---

## 6. Retrieval Method per Source
...
| DPA Legal Trigger Matrix (DPA-TM-001) | Indexed: hybrid dense vector + BM25, row-targeted | Row-level chunking; each row = one atomic chunk; column headers embedded in each chunk | Row structure is preserved during chunking so rows remain interpretable as complete decision units. |
| Procurement Approval Matrix (PAM-001) | Indexed: hybrid dense vector + BM25, row-targeted | Row-level chunking; same as DPA matrix | Same row-preservation approach. Eligible for structured lookup upgrade in a future version. |
| Vendor Questionnaire (VQ-OC-001) | Direct structured access — JSON field lookup by key | No chunking. Ingested as a single structured JSON object at intake. | Fields are read by canonical key name (see §15), not by similarity search. Never embedded or indexed. |
| Prior Vendor Decisions (PVD-001) | Indexed: hybrid dense vector + BM25 with metadata filtering | Record-level chunking; one prior decision = one chunk with domain metadata tag | Domain-scoped at access time (security decisions to Security Agent, etc.). Treat as Tier 3. |
...
### 6.1 Retrieval Endpoint Permissions

Retrieval endpoint permissions define which index endpoints each agent is authorized to query. An agent that attempts to query an index it is not authorized to access must fail closed; the attempt is logged per §14.2.
...
| idx_precedents (domain-scoped) | ✓ Security scope only | ✓ Legal scope only | ✓ Procurement scope only | — No access | — No access |
| idx_slack_notes (Procurement-scoped) | — No access | — No access | ✓ Procurement threads only | — No access | — No access |
...
**PRIVILEGED DOCUMENT EXCLUSION:** Attorney-client privileged communications are excluded from all indices. No index endpoint for privileged communications exists.

---

## 7. Freshness & Versioning Policy by Source Class

| SOURCE CLASS | VERSION PINNING | RUN-TIME REFRESH | STALE-BUT-USABLE | PROVISIONAL USE | HUMAN RECONFIRMATION |
|--------------|-----------------|------------------|------------------|-----------------|----------------------|
| Static versioned policy (ISP-001, DPA-TM-001, PAM-001) | Pinned at initialization. Version ID locked to manifest entry. | Not eligible for refresh during run. | Not permitted. Unverified version triggers PROVISIONAL evidence flag. | Permitted only when `manifest_status = PROVISIONAL` and owning domain has been notified. | Required before `overall_status = COMPLETE` when source is PROVISIONAL. |
| Structured intake (VQ-OC-001) | Pinned at submission. If multiple versions exist: `workflow_state = BLOCKED`, `resolution_mode = ESCALATED_TO_PROCUREMENT`. | Not eligible. Questionnaire is immutable once intake validation passes. | Not applicable. | Not applicable. | Required if version conflict detected. |
| Governed precedent (PVD-001) | Pinned to state at initialization. New precedents added after init are out of scope for this run. | Not eligible during run. | Permitted. A dated record is usable as context with a recency note in the retrieval manifest. | Permitted without triggering run-level PROVISIONAL. | Not required unless precedent is the only evidence for a determination. |
| Low-authority supplemental (SLK-001) | Captured as export at initialization. No run-time update. | Not eligible. | Permitted. Recency noted in retrieval manifest but not blocking. | Not applicable — Tier 4 sources never drive determinations. | Not required. |

---

## 8. Evidence Provenance Minimum Requirements
...
| `source_type` | One of: `POLICY` \| `MATRIX` \| `QUESTIONNAIRE` \| `PRECEDENT` \| `SUPPLEMENTAL_NOTE`. Required for all source types. |
...
| `authority_tier` | Integer 1–4, matching the tier in §5. Required for all source types. |
| `citation_class` | One of: `PRIMARY` \| `SUPPLEMENTARY`. Tier 3–4 sources are always SUPPLEMENTARY. Required for all source types. |
...
| `manifest_status` | Status of the source in the initialization manifest: `CONFIRMED` \| `PROVISIONAL`. Propagates PROVISIONAL evidence flag to dependent determinations. |
...
## 9. Retrieval Bundle Admissibility Requirements
...
| Risk classification matrix row | Required if ERP integration type is identified. If ERP type is ambiguous, classification evidence is insufficient; determination carries PROVISIONAL flag. |
| Precedent context | Optional. Include if a relevant security-domain precedent exists. |
| Slack / notes | Excluded per §6.1. |

### 9.2 Legal Agent Bundle
...
| Precedent context — legal domain | Optional. |
| Slack / notes | Excluded per §6.1. |

### 9.3 Procurement Agent Bundle
...
| Raw source documents | Excluded. Checklist Assembler receives only processed agent outputs and audit log data. |

### 9.5 Checkoff Agent Bundle
...
| Any index endpoint query | Prohibited. The Checkoff Agent is downstream-only. |

---

## 10. Token Budget per Agent
...
### 10.1 Priority Ordering Under Budget Constraint
...
5. Supplementary context (Tier 4 — Slack / notes). Excluded first when budget is constrained.

---

## 11. Low-Authority Source Handling Rules

Slack threads and meeting notes (SLK-001) are governed by the following rules in addition to the authority hierarchy in §5.

| RULE | SPECIFICATION |
|------|---------------|
| Authority score cap | Tier 4 sources receive a fixed authority weight cap at re-ranking, regardless of semantic similarity score. A Tier 4 chunk with a higher similarity score than a Tier 1 chunk is still weighted below the Tier 1 chunk in the final bundle. |
| Citation class restriction | Tier 4 sources may only appear as `citation_class: SUPPLEMENTARY`. They may never appear as `citation_class: PRIMARY`. |
| Conflict suppression | When a Tier 4 source conflicts with any Tier 1, 2, or 3 source, the Tier 4 source is suppressed from the bundle. Logged in the retrieval manifest with reason: `AUTHORITY_SUPPRESSED`. |
| Intra-Tier 4 conflict | When two Tier 4 threads conflict with each other, both are suppressed. If Tier 4 is the only available evidence on a point, the evidence is insufficient for a RESOLVED determination. |
| Access scope | Only Procurement-scoped threads are eligible. No other agent may access Tier 4 sources. |
| Inclusion gate | Tier 4 evidence is included only when it adds non-conflicting, non-redundant context beyond what Tier 1–3 sources provide. |
| Determination driver prohibition | Tier 4 evidence is never the sole or primary basis for a RESOLVED determination. It may supplement a RESOLVED determination but may not constitute it. |

---

## 12. Determination Evidence Sufficiency Thresholds

These thresholds define when retrieved evidence is sufficient to support a given determination status. This is a retrieval governance rule about evidence adequacy. How the pipeline emits and routes statuses is governed by the Design Doc (§10) and Agent Spec.

| DETERMINATION STATUS | EVIDENCE REQUIREMENTS |
|---------------------|----------------------|
| **RESOLVED** (evidence sufficient) | At least one Tier 1 source citation supporting the determination. Questionnaire fields relevant to the determination are present and unambiguous. No unresolved conflicting Tier 1 evidence exists. If any governing source carries `manifest_status = PROVISIONAL`, the determination is RESOLVED-PROVISIONAL rather than RESOLVED-CONFIRMED. |
| **PROVISIONAL** (evidence present but incomplete) | Governing source is queryable but version-unconfirmed, OR only Tier 3 evidence is present without Tier 1 corroboration, OR a required questionnaire field is ambiguous. The evidence chain has a gap, not a conflict. |
| **ESCALATED** (evidence present but conflicting or out-of-scope) | Required evidence is present but conflicts between two or more Tier 1 sources, OR the evidence pattern falls outside the defined rule set of the governing matrix, OR human judgment is explicitly required by policy. See §3.2. |
...
### 12.1 Specific Thresholds by Determination Type

| DETERMINATION TYPE | MINIMUM EVIDENCE THRESHOLD |
|-------------------|---------------------------|
| Data classification (IT Security Agent) | At least one IT Security Policy section citation plus questionnaire ERP integration type confirmed. If ERP type is ambiguous: evidence is insufficient for RESOLVED; determination is PROVISIONAL. |
...
| Approval path routing (Procurement Agent) | Both IT Security and Legal outputs must be present (RESOLVED or RESOLVED-PROVISIONAL). At least one procurement matrix row match required. If no row matches: evidence is insufficient for RESOLVED. |
...
## 13. Escalation Ownership

This section governs escalation conditions triggered by retrieval or evidence quality issues. Workflow routing in response to escalation is governed by the Design Doc (§3). BLOCKED/ESCALATED convention follows §3.2.
...
| Data classification ambiguous (ERP type field unclear) | Evidence insufficient for RESOLVED classification (PROVISIONAL pending field confirmation) | IT Security (K. Whitfield or delegate) |
...
| ERP integration tier unclassified (ISP-001 §12.2 amber row) | Evidence insufficient for RESOLVED classification (PROVISIONAL; unclassified row creates ambiguity) | IT Security |
| NDA status unconfirmed (ISP-001 §12.1.4) | Evidence insufficient to confirm NDA requirement satisfied (PROVISIONAL on NDA-dependent conditions) | Procurement |
...
### 13.1 Minimum Escalation Evidence Payload
...
| DPA requirement determination | PRIMARY: at least one DPA trigger matrix row (DPA-TM-001, row ID, version, trigger condition). SUPPLEMENTARY: upstream classification output reference (agent_id, pipeline_run_id). No Tier 3 or 4 source may be cited as PRIMARY. |
...
| Approval path routing | PRIMARY: procurement approval matrix row (PAM-001, row ID, approval path condition). SUPPLEMENTARY: both upstream domain agent outputs cited by agent_id. Tier 4 note permitted as SUPPLEMENTARY only if non-conflicting. |
...
| PROVISIONAL flagging | The source or field triggering the PROVISIONAL flag must be identified by `source_id`, `version`, and `chunk_identifier` (or field path). The provisional condition must be stated explicitly, not as a general disclaimer. |
...
### 14.1 Audit Log Citation Requirements by Event Type
...
| STATUS_CHANGE | Reference to the triggering determination or retrieval event. For PROVISIONAL: the source or field that triggered the evidence flag. For ESCALATED: both conflicting sources with `chunk_identifiers`. For BLOCKED: the missing required evidence type. |
| ESCALATION | Full escalation payload per §13.1 including `triggering_source` and `conflicting_evidence` with `chunk_identifiers`. |

### 14.2 Audit Log Event Triggers
...
- Every evidence-layer status change: PROVISIONAL flag set or cleared, ESCALATED condition identified, BLOCKED condition identified
- Every determination emitted by a domain agent

---

## 15. Source Normalization and Canonical Field Mapping
...
| `nda_status` | NDA status: `EXECUTED` \| `PENDING` \| `NOT_STARTED` \| `UNKNOWN` | VQ-OC-001 | `existing_nda_status` (JSON); NDA confirmation clause (ISP-001 §12.1.4) |
...
| `approval_path` | Routing outcome: `STANDARD` \| `FAST_TRACK` \| `EXECUTIVE_APPROVAL` | Procurement Agent output | `approval_path` (PAM-001 rows); "Approval Route" (Procurement matrix) |
...
## 16. Contract Lock and Change Protocol
