...
- [ ] **`status`** — see general rules. Expected values by scenario:
  ...
  - Scenario 4 (no upstream STEP-02 output in the bundle): `blocked` — pure gate-condition test per Agent Spec §8.5 ("upstream_data_classification absent" → `blocked`, output shape switching rule) and §12 ("data_classification absent from STEP-02 output | Bundle is inadmissible. Emit the §9.1 blocked output shape."). On blocked runs, the agent must emit the §9.1 blocked output shape: `{"status": "blocked", "blocked_reason": [...], "blocked_fields": [...]}`. Determination fields (`dpa_required`, `dpa_blocker`, `nda_status`, `nda_blocker`, `trigger_rule_cited`, `policy_citations`) must be entirely **absent** from the output — not null, not empty. Absent means the agent declined to produce a determination it had no basis to make; null implies the field exists but has no value. `blocked_reason` must be a non-empty array of enum values (e.g., `MISSING_UPSTREAM_IT_SECURITY_OUTPUT`). `blocked_fields` must be a non-empty array of canonical field names from CC-001 §15 that were absent upstream (e.g., `data_classification`). Any determination field present in the output — regardless of value — is a hard failure. Designed to catch models that infer through missing input rather than halting.
